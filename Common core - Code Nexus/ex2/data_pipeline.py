...
def main() -> None:
    ...
    num.ingest([3.14, -1, 2.71])

    text.ingest(["Hello world", "Hi", "five"])
    ...
    csv.process_output([num.output() for _ in range(3)])
    csv.process_output([text.output() for _ in range(3)])
    csv.process_output([log.output() for _ in range(2)])
    ...
    json.process_output([num.output() for _ in range(3)])
    json.process_output([text.output() for _ in range(3)])
    json.process_output([log.output() for _ in range(2)])
